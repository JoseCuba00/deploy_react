...
const SelectBlock = ({ objects, ClickFunction,boxIndex, disabled, selectedChoices }) => {
    console.log(selectedChoices)
    return (
        <div className='d-inline-flex'>
            <span> (</span>
            {objects[boxIndex].map((obj, index) => {
                const isSelected = selectedChoices.some(selectedChoice => selectedChoice.id === obj.id)
                return(
                <div className="d-flex p-0 " key={index}>

                    <input onClick={() => ClickFunction(boxIndex, obj)} type="radio" className="btn-check" name={`options-outlined${boxIndex}`} id={`radio-${obj.id}`} disabled={disabled} />

                    <label class={` select-quiz ${isSelected && `selected ${disabled && 'isDisabled'}`} `} htmlFor={`radio-${obj.id}`}
                ...
                props.list.map((sentence, boxIndex) => {
                    ...
                    return (
                        props.isText ?
                            ...
                            <div key={boxIndex}>
                                <span>{boxIndex + 1} - &nbsp;{firstPart}</span>
                                <SelectBlock
                                    objects={props.objects}
                                    ClickFunction={props.ClickFunction}
                                    boxIndex={boxIndex}
