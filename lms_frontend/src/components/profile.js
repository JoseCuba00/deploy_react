...
function Profile() { // Esto es un componente, es un pedazo de UI , es como un div
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const response = await fetch(`http://127.0.0.1:8000/api/get_profile_image/${user.id}`)
    ...
    const onFileChange = async (event) => {
        ...
        try {
            const response = await axios.put(`http://127.0.0.1:8000/api/profile_image/${user.id}`, formData, {
                headers: {
    ...
    return (
        ...
        isLoading ? <p>Loading</p> : <div className='container pt-5 d-flex justify-content-evenly pt-5' style={{ border: '1px solid transparent', background: 'white', position: 'relative', top: '25px' }} >

            <div className='d-flex flex-column'>
                <img className='img_profile ' src={userInfo[0].profile_image}></img>
                <h5 style={{ fontWeight: 600, paddingTop: '23px' }}> Personal information </h5>
                ...
                <div className='d-flex pb-4'>
                    <div className='pe-3'>
                        <label htmlFor="file-upload" className="btn btn-primary my-2 " style={{ background: '#341ca6', border: '1px solid transparent' }}>
                            Change profile photo
                            <input id="file-upload" type="file" style={{ 'display': 'none' }} onChange={(event) => onFileChange(event)} />
                        </label>
                    </div>
                    <div>
                        <Link to='change_password' className="btn btn-primary my-2  " type="submit" style={{ background: '#341ca6', border: '1px solid transparent' }} >Change password</Link>
                    </div>
                </div>
